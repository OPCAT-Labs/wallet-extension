...
// Determine the correct app root path
// If we're in a monorepo (running from root), use packages/extension
// If we're directly in the extension package, use current directory
let appRoot;
if (fs.existsSync(path.join(process.cwd(), 'packages/extension'))) {
  // Running from monorepo root
  appRoot = fs.realpathSync(path.join(process.cwd(), 'packages/extension'));
} else {
  // Running directly from extension package
